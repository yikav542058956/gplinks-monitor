...
SESSION_STR = os.environ.get("TELEGRAM_SESSION", "")  # String session or file path
...
async def send_to_monitor_chat(client: TelegramClient, message: str):
    try:
...
async def process_link(client: TelegramClient, url: str):
    ...
    try:
        await send_to_monitor_chat(client, f"⚡ <b>Processing...</b>\n<code>{url[:50]}...</code>")
        dest = await bypass_gplinks(url)
        await send_to_monitor_chat(client, f"✅ <b>Done!</b>\n🔗 <code>{dest}</code>\n<a href='{dest}'>Open →</a>")
        logger.info(f"✅ {dest[:80]}")
    except Exception as e:
        if "DEST_SERVER_DOWN" in str(e):
            await send_to_monitor_chat(client, "⚠️ <b>Destination DOWN!</b>\nAll steps complete, server offline.")
        else:
            await send_to_monitor_chat(client, f"❌ {str(e)[:200]}")
